...
export const getSeats: RequestHandler = (_, res: TypedRes<GetSeatsRes>) => {
  const jsonFile = fs.readFileSync(`${JSON_DIRECTORY}/${CURRENT_SEATS}`, 'utf8');
  const parsedJSON = JSON.parse(jsonFile);
  return res.send(parsedJSON);
};

export const getLastWeekSeats: RequestHandler = (_, res: TypedRes<GetLastWeekSeatsRes>) => {
...
export const cancelReservation: RequestHandler = (
